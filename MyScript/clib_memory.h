    ...
    namespace memory
    {
        ...
        class legacy_memory_pool
        {
            ...
            bool _free(void* p)
            {
                ...
                --blk; // 自减得到块的元信息头
                return _free_block(blk);
            }

            // 释放内存
            bool _free_block(block *blk)
            {
            ...
            // 标记
            bool _mark(void* p, bool marked)
            {
                block *blk = static_cast<block*>(p);
                --blk; // 自减得到块的元信息头
                if (!verify_address(blk))
                    return false;
                block_set_flag(blk, BLOCK_MARKED, marked);
                return true;
            }

            // 垃圾回收
            template<class T>
            void _gc_collect(std::function<T> hook)
            {
                if (block_current == block_head)
                    return;
                auto blk = block_current;
                do
                {
                    auto next_blk = blk->next;
                    if (!block_get_flag(blk, BLOCK_MARKED))
                    {
                        hook(static_cast<T*>(blk + 1));
                        _free_block(blk);
                    }
                    else
                    {
                        block_set_flag(blk, BLOCK_MARKED, 0);
                    }
                    blk = next_blk;
                } while (blk != block_current);
            }
        ...
        public:
            ...
            bool mark(T* obj)
            {
                return _mark(obj, true);
            }

            template<class T>
            bool unmark(T* obj)
            {
                return _mark(obj, false);
            }

            template<class T>
            void gc(std::function<T> hook)
            {
                return _gc_collect(hook);
            }
